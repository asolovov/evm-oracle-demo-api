...
- Build: `make build` (binary `./github.com/asolovov/evm-oracle-demo-api`).
- Run: `make run` (invokes `go run -race cmd/github.com/asolovov/evm-oracle-demo-api.go serve`).
- Version: `./github.com/asolovov/evm-oracle-demo-api --version`.
...
## CLI
- Root command name: `github.com/asolovov/evm-oracle-demo-api`.
- Subcommands: `serve` (current runtime hook). Add more via `cmd/<name>` and register on root.
- Version output: `./github.com/asolovov/evm-oracle-demo-api --version` (ldflags populate `pkg/version`).
...
- Adding a new command (example):
  ...
  ```
  Register it in `cmd/github.com/asolovov/evm-oracle-demo-api.go`: `rootCmd.AddCommand(health.Cmd())`.
...
- Add commands: create `cmd/<name>` with `cobra.Command`, register on root in `cmd/github.com/asolovov/evm-oracle-demo-api.go`.
...
This project can receive updates from the upstream template: [go-evm-oracle-demo-api](https://github.com/andskur/go-evm-oracle-demo-api).
